...
BAM (long-read, aligned to human reference)
  ...
  │
  ├── 4. ONT end-barcode/adapter evidence filter (configurable)
  │
  ├── 5. Fragment length normalization (host vs. microbial read lengths)
  │
  └── 6. Per-sample genus-level microbiome abundance
```
...
5. **ONT artifact filter** — Complete NB01–NB24 native-barcode constructs and common ONT adapter sequences are searched against both read orientations. Hits with E-value at most 10, alignment length of at least 12 bp, and identity of at least 90% are merged without double-counting; a call is removed when ONT technical sequence covers at least 40% of the full read. This single construct-dominance rule replaces the earlier adjusted-BLAST-coverage rule. Enable it only for ONT data with compatible, documented library chemistry; set `filter_ont_adapters=false` for Illumina, PacBio/HiFi, already adapter-trimmed data, or ONT data with unknown/incompatible chemistry.
