prefix=$1
DB=$2
cat $prefix.krakenuniq|awk '$1=="C"' > $prefix.krakenuniq.classified 
cat $prefix.classified_by_krakenuniq |paste - - > $prefix.classified_by_krakenuniq.convert
...
cat $prefix.krakenuniq.info_collection|awk '/d__|k__/ && !/synthetic/ && !/Homo_sapiens/ && !/Homo sapiens/' \
	> $prefix.krakenuniq.info_collection.flt
cat $prefix.krakenuniq.info_collection.flt|awk '{print $1"\n"$2}' > $prefix.krakenuniq.microbiome.fasta
